...
export interface SearchSuggestBoxProps {
   ...
   * "navbar" renders a bare, unstyled input with a trailing dark "Search"
   * button - no border/pill of its own - so the desktop Navbar can wrap it
   * together with the category-scope dropdown in one shared pill.
   */
  variant?: "default" | "amazon" | "navbar";
}
...
export function SearchSuggestBox({
  ...
  if (variant === "navbar") {
    return (
      <div ref={containerRef} className={cn("relative h-full min-w-0 flex-1", className)}>
        <form
          role="search"
          onSubmit={(e) => {
            e.preventDefault();
          ...
          className="flex h-full min-w-0 items-center pl-4 pr-1"
        >
          <input
            ref={inputRef}
            type="search"
            ...
            autoComplete="off"
            className="min-w-0 flex-1 appearance-none border-0 bg-transparent text-[14px] text-ink outline-none ring-0 placeholder:text-neutral-400 focus:outline-none focus:ring-0 [&::-webkit-search-cancel-button]:hidden"
          />

          <button
            type="submit"
            aria-label="Search"
            className="ml-2 inline-flex h-[calc(100%-8px)] shrink-0 items-center gap-1.5 rounded-full bg-ink px-5 text-[13px] font-semibold text-paper transition-colors hover:bg-neutral-800"
          >
