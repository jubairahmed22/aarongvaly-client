...
export default async function ProductPage({ params }: PageProps) {
  ...
  // Full ancestor chain (root → immediate parent) plus the product's own
  // category, so multi-level trees render as "Home > Fruits & Vegetables >
  // Fresh Vegetables > Sweet Potato" instead of collapsing to one level.
  const crumbs = [
    { label: "Home", href: "/" },
    ...(categorySummary?.ancestors ?? []).map((a) => ({
      label: a.name,
