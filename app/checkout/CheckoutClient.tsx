...
export function CheckoutClient() {
  ...
  if (!cart || cart.items.length === 0) {
    return (
      <div className="mt-[24px] flex flex-col items-center gap-[12px] rounded-xl border border-neutral-200 bg-white px-[24px] py-[56px] text-center">
        <p className="text-[16px] font-semibold text-ink">Your cart is empty</p>
        <p className="text-[14px] text-neutral-500">Add a few items before checking out.</p>
        <Link href="/all-products" className={buttonVariants({ variant: "accent", size: "md", className: "mt-[4px] rounded-xl" })}>
          Browse products
        </Link>
      </div>
    );
  ...
  return (
    ...
    <div className="pb-24 sm:pb-0">
      <h1 className="text-[22px] font-extrabold text-ink sm:text-[26px]">Checkout</h1>
      ...
      <div className="flex flex-col gap-[16px]">
        ...
        <section className="flex flex-col gap-2.5 rounded-xl border border-neutral-200 bg-white p-[16px]">
          <h2 className="text-[16px] font-bold text-ink">Order note <span className="text-sm font-normal text-neutral-400">(optional)</span></h2>
          <textarea
            value={customerNote}
            ...
            rows={3}
            maxLength={1000}
            className="w-full rounded-lg border border-neutral-300 bg-neutral-50 px-[12px] py-[10px] text-[14px] text-ink placeholder:text-neutral-400 focus:border-accent focus:bg-white focus:outline-none"
          />
        </section>
      </div>
    ...
    <div className="fixed bottom-0 left-0 right-0 z-30 flex items-center gap-[12px] border-t border-neutral-200 bg-white px-[16px] pt-[12px] shadow-[0_-4px_16px_rgba(0,0,0,0.08)] sm:hidden" style={{ paddingBottom: "max(0.75rem, env(safe-area-inset-bottom))" }}>
      <div className="min-w-0 flex-1">
        <p className="text-[12px] text-neutral-500">Total</p>
        <p className="text-[16px] font-bold text-ink">{formatPrice(total, cart.currency)}</p>
      </div>
      <Button
...
function ShippingAddressBlock({ addresses, selectedId, onSelect, form, showEmail }: ShippingAddressBlockProps) {
  return (
    <section className="flex flex-col gap-3 rounded-xl border border-neutral-200 bg-white p-[16px]">
      <h2 className="text-[16px] font-bold text-ink">Shipping address</h2>
        ...
        <ul className="flex flex-col gap-2">
            ...
            return (
              <li key={a._id}>
                <button
                  ...
                  className={cn(
                    "flex w-full items-start gap-3 rounded-lg border p-[12px] text-left transition-colors",
                    active ? "border-accent bg-accent/5" : "border-neutral-200 hover:border-neutral-400",
                  )}
                >
                  ...
                  <span
                    className={cn(
                      ...
                      active ? "border-accent" : "border-neutral-300",
                    )}
                  >
                  ...
                  <div className="flex-1 text-[14px]">
                    ...
                    <div className="mt-0.5 text-neutral-500">{a.phone}</div>
          ...
          <li>
            <button
              ...
              className={cn(
                "inline-flex items-center gap-2 rounded-lg border border-dashed px-[12px] py-[10px] text-[14px] font-medium transition-colors",
                selectedId === "new" ? "border-accent bg-accent/5 text-accent" : "border-neutral-300 text-neutral-500 hover:border-neutral-400 hover:text-ink",
              )}
            >
    ...
    <form className="grid grid-cols-1 gap-1.5 sm:grid-cols-2" onSubmit={(e) => e.preventDefault()}>
      ...
      {showSaveToggle ? (
        <label className="flex items-center gap-2 text-sm text-ink sm:col-span-2">
          <input type="checkbox" {...register("saveAddress")} className="h-[16px] w-[16px] accent-accent" />
...
function Field({ label, required, error, className, children }: FieldProps) {
  return (
    <div className={cn("flex flex-col gap-0.5", className)}>
      <Label required={required} className="text-[13px] font-medium text-neutral-600">{label}</Label>
      {children}
          ...
          return (
            <li key={opt.id}>
              <button
                ...
                className={cn(
                  "flex w-full items-center gap-3 rounded-lg border p-[12px] text-left transition-colors",
                  active ? "border-accent bg-accent/5" : "border-neutral-200 hover:border-neutral-400",
                )}
              >
                <span
                ...
                <div className="flex-1 text-[14px]">
                  ...
                  <div className="text-[12px] text-neutral-500">{opt.description}</div>
                </div>
...
function OrderSummary({
  ...
  return (
    <aside className="flex flex-col gap-3 self-start rounded-xl border border-neutral-200 bg-white p-[16px] md:sticky md:top-20">
      <h2 className="text-[16px] font-bold text-ink">Order summary</h2>

      <ul className="flex flex-col divide-y divide-neutral-100">
        {cart.items.map((it) => {
          const onSale = typeof it.originalPrice === "number" && it.originalPrice > it.price;
          return (
          <li key={it._id} className="flex items-start gap-2.5 py-2.5 first:pt-0 last:pb-0">
            <div className="relative h-[48px] w-[48px] shrink-0 overflow-hidden rounded-lg border border-neutral-200 bg-neutral-50">
              {it.image ? (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-1">
                ...
                <button
                  ...
                  className="mt-0.5 shrink-0 rounded p-0.5 text-neutral-400 transition-colors hover:text-red-600"
              ...
              <div className="mt-1.5 flex items-center justify-between">
                <div className="flex h-[26px] items-center overflow-hidden rounded-[6px] bg-accent">
                  ...
                  >
                    <Minus className="h-[10px] w-[10px]" strokeWidth={3} />
                  </button>
                  <span className="min-w-[20px] select-none text-center text-[11px] font-bold text-white">{it.qty}</span>
                  <button
                    type="button"
                  ...
                  >
                    <Plus className="h-[10px] w-[10px]" strokeWidth={3} />
                  </button>
                </div>
                <div className="flex items-baseline gap-[6px]">
                  <span className="text-sm font-bold text-green-700">{formatPrice(it.price * it.qty, cart.currency)}</span>
                  {onSale ? (
                    <span className="text-[11px] text-neutral-400 line-through">
                      {formatPrice(it.originalPrice! * it.qty, cart.currency)}
                    </span>
                  ) : null}
                </div>
              </div>
      ...
      <div className="flex flex-col gap-1.5 border-t border-neutral-100 pt-3 text-sm">
        ...
        {appliedCoupon ? (
          <div className="flex justify-between">
            <span className="text-neutral-500">Coupon ({appliedCoupon.code})</span>
            <span className="font-semibold text-green-700">−{formatPrice(discount, cart.currency)}</span>
          </div>
      ...
      <Button onClick={onPlaceOrder} loading={isPlacing} variant="accent" size="md" fullWidth className="rounded-xl">
...
function Row({ label, value, muted }: { label: string; value: string; muted?: boolean }) {
  return (
    <div className="flex justify-between">
      ...
      <span className={cn("font-medium text-ink", muted && "font-normal text-neutral-500")}>{value}</span>
    </div>
  );
}
...
function AddOnBreakdown({
  ...
  return (
    <div className="mt-1 flex flex-col gap-px rounded-sm border border-neutral-100 bg-neutral-50 px-1.5 py-1 text-[11px]">
      ...
      <div className="flex justify-between border-t border-neutral-200 pt-px font-medium text-ink">
        <span>Per item</span>
