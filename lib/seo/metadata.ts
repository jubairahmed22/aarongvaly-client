 ...
 *    defines a `%s · ${SITE_NAME}` template; using `absolute` means THIS module
...
/** Append " · {brand}" unless the title already references the brand. */
...
export function categoryMetadata(c: CategoryDetail): Metadata {
  return buildMetadata({
    ...
    description:
      c.metaDescription ??
      c.description ??
      `Shop ${c.name} at ${SITE_NAME} - genuine products, great prices and fast delivery nationwide.`,
...
export function brandMetadata(b: BrandDetail, path?: string): Metadata {
  return buildMetadata({
    ...
    description:
      b.metaDescription ??
      b.description ??
      `Shop ${b.name} at ${SITE_NAME}. Genuine products and fast delivery nationwide.`,
...
export function homeMetadata(): Metadata {
  return buildMetadata({
    title: `${SITE_NAME} - Online Shopping for Everything, Delivered Fast`,
    description:
