...
export interface NotificationsBellProps {
  /** Shrinks the trigger for use in the thin utility bar (vs. the main icon row). */
  compact?: boolean;
}
  ...
  return (
    <div ref={rootRef} className="relative inline-block">
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className={cn(
          "relative inline-flex items-center justify-center rounded-full text-paper transition-colors hover:bg-white/10",
          compact ? "h-[26px] w-[26px]" : "h-[40px] w-[40px]",
        )}
      >
        <Bell className={compact ? "h-[15px] w-[15px]" : "h-[22px] w-[22px]"} aria-hidden />
        {unreadCount > 0 ? (
          <span
            aria-hidden
            className="absolute -right-0.5 -top-0.5 inline-flex h-2 w-2 items-center justify-center rounded-full bg-accent px-1 text-[10px] font-semibold leading-4 text-ink tabular-nums"
          >
